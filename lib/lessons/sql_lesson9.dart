...
class SqlLesson9 extends StatefulWidget {
  const SqlLesson9({super.key});

  @override
  State<SqlLesson9> createState() => _SqlLesson9State();
}

class _SqlLesson9State extends State<SqlLesson9> {
  ...
  void loadStatus() async {
    ...
    setState(() {
      isCompleted = prefs.getBool("Lesson 9 — UNION vs UNION ALL") ?? false;
    });
  }

  Future<void> markCompleted() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool("Lesson 9 — UNION vs UNION ALL", true);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Lesson 9 — UNION vs UNION ALL")),
        ...
        child: Column(
          ...
          children: [
            ...
            DataTable(
              ...
              rows: const [
                DataRow(cells: [DataCell(Text("John"))]),
                DataRow(cells: [DataCell(Text("Mary"))]),
              ],
            ),
            ...
            DataTable(
              ...
              rows: const [
                DataRow(cells: [DataCell(Text("Mary"))]),
                DataRow(cells: [DataCell(Text("David"))]),
              ],
            ),
