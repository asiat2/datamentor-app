...
class PythonLesson6 extends StatefulWidget {
  const PythonLesson6({super.key});

  @override
  State<PythonLesson6> createState() => _PythonLesson6State();
}

class _PythonLesson6State extends State<PythonLesson6> {
  ...
  void loadStatus() async {
    ...
    setState(() {
      isCompleted = prefs.getBool("Lesson 6 — NumPy") ?? false;
    });
  }

  Future<void> markCompleted() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool("Lesson 6 — NumPy", true);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Lesson 6 — NumPy"), centerTitle: true),
